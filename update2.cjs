...
<body>
  ...
  <script>
    ...
    els.btnCancel.onclick = () => {
      ...
      socket.emit('cancel-transfer'); // Optionally tell server
    };
    ...
    socket.on('file-staged', (data) => {
      ...
      els.btnCancel.classList.add('hidden'); // No cancel for direct downloads
...
content = before + `getMobileHtml() {\n    return \`${newScript}\`;\n  }\n\n  ` + after;

// Add this.io.emit to cancelTransfer if not present
if (!content.includes("this.io.emit('transfer-cancelled')")) {
